...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_char_padded = pad_sents_char(sents_char, self.char2id['<pad>'])
		# size (batch_size, max_sentence_length, max_word_length)
        tensor_sents_char_padded = torch.tensor(sents_char_padded, dtype=torch.long, device=device)
		
        return tensor_sents_char_padded.permute(1, 0, 2)
        ### END YOUR CODE
